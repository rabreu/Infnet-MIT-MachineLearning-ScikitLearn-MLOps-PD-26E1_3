...
# %%
if check_kaggle_credentials(secrets_path=str(SECRETS_PATH)):
...
raw_dir = pipeline_cfg.get('paths').get('raw_data_dir')
data_path = Path(ROOT_DIR / raw_dir)
...
processed_dir = pipeline_cfg.get('paths').get('processed_data_dir')
output_path = Path(processed_dir + '/' + pipeline_cfg.get('paths').get('output_dir'))
output_path.mkdir(parents=True, exist_ok=True)
#
output_filename = pipeline_cfg.get('paths').get('output_filename')
...
logger.info('Arquivos pronto: %s', len(downloaded))

datasets = load_datasets(expected_files=expected_files,
                         data_path=data_path,
                         logger=logger
)
...
engine = pipeline_cfg.get('execution').get('engine')
