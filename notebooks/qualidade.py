...
output_dir = processed_dir / config.get('paths').get('output_dir')
parquet_path = output_dir / config.get('paths').get('output_filename')
engine = pipeline_config.get('execution').get('engine')
compression = data_config.get('ingest').get('compression')
...
# df = pd.read_parquet(parquet_path, engine=engine, compression=compression)
# df = pq.read_table(parquet_path, read_dictionary=schema, thrift_string_size_limit=2147483647, thrift_container_size_limit=2147483647, use_threads=True).to_pandas()
df = pq.read_table(parquet_path, schema=schema).to_pandas()
# df = pq.read_table(parquet_path, schema=schema).to_pandas()
