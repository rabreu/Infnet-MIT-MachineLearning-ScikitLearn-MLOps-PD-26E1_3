...
All parameters are read from config/olist.yaml — no hardcoded values.
"""
...
# ---------------------------------------------------------------------------
# Path bootstrap — allows running the script directly from the olist/ directory
# ---------------------------------------------------------------------------
...
def run(config: dict, base_dir: Path) -> pd.DataFrame:
    ...
    Args:
        config:   Full config dict loaded from config/olist.yaml.
        base_dir: Repository root (parent of olist/, config/, outputs/).
    ...
    log_file_rel = logging_cfg.get("log_file", "outputs/olist.log")
    logging_cfg = {**logging_cfg, "log_file": str(base_dir / log_file_rel)}
    logger = get_logger("olist.feature_engineering", logging_cfg)
...
if __name__ == "__main__":
    ...
    _cfg_path  = _base_dir / "config" / "olist.yaml"
