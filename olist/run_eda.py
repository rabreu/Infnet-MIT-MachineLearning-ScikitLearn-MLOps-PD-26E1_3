...
# From the aula02 directory:
    python olist/run_eda.py
    python olist/run_eda.py --steps descriptive pivot_tables
    python olist/run_eda.py --config config/ --base-dir .
    python olist/run_eda.py --steps all --force
...
# ── Path bootstrap ─────────────────────────────────────────────────────────────
# Allow running from: aula02/, olist/, or anywhere via absolute path
...
logger = get_logger("olist.orchestrator", _DEFAULT_LOG_CFG)

# ── Module registry ────────────────────────────────────────────────────────────
# Ordered list of (step_name, module_path, import_name)
_STEPS: list[tuple[str, str]] = [
    ("descriptive",         "olist.descriptive"),
    ("visualizations",      "olist.visualizations"),
    ("pivot_tables",        "olist.pivot_tables"),
    ("statistical_tests",   "olist.statistical_tests"),
    ("interactions",        "olist.interactions"),
    ("feature_engineering", "olist.feature_engineering"),
    ("clustering",          "olist.clustering"),
]
...
def run_eda(
    ...
    Args:
        config_dir: Directory containing pipeline.yaml, data.yaml, and olist.yaml.
    ...
    # Load olist-specific config (olist.yaml may need separate load since load_config merges pipeline+data)
    eda_yaml_path = config_dir / "olist.yaml"
    ...
    run_results: dict[str, Any] = {
        "pipeline": config.get("olist", {}).get("name", "california-housing-olist"),
        "version":  config.get("olist", {}).get("version", "1.0.0"),
