...
def _add_binned_columns(df: pd.DataFrame, config: dict, logger: Any) -> pd.DataFrame:
    ...
    for new_col, (src_col, cfg_key) in bin_map.items():
        ...
        if cfg_key not in bins_cfg:
            logger.warning("Bin config '%s' missing from olist.yaml — skipping '%s'.", cfg_key, new_col)
            continue
...
def run(config: dict, base_dir: Path) -> dict:
    ...
    Args:
        config:   Merged configuration dict loaded from olist.yaml.
    ...
    logger = get_logger("olist.pivot_tables", config.get("logging", {}))
...
if __name__ == "__main__":
    ...
    _CONFIG_PATH = _BASE_DIR / "config" / "olist.yaml"
    if not _CONFIG_PATH.exists():
        raise FileNotFoundError(f"olist.yaml not found at {_CONFIG_PATH}")
