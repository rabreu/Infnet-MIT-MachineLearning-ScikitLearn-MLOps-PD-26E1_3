...
are driven by config/olist.yaml; nothing is hardcoded in this module.
...
def _bootstrap_logger(config: dict) -> logging.Logger:
    ...
    return get_logger("olist.visualizations", logging_cfg)
...
def run(config: dict, base_dir: Path) -> list[str]:
    ...
    Args:
        config:   Loaded configuration dict (from config/olist.yaml).
